...
contract PayoutFuzzTest is BaseTest {
    ...
    function testFuzz_PayoutCalculation(
        ...
        // Place bets
        placeBet(alice, marketId, HashPrediction.Outcome.Yes, yesBetAmount);
        placeBet(bob, marketId, HashPrediction.Outcome.No, noBetAmount);

        // Resolve
        warpToResolution(marketId);
        HashPrediction.Outcome winningOutcome =
    ...
    function testFuzz_MultipleBettors(
        ...
        for (uint256 i = 0; i < 3; i++) {
            if (yesBets[i] > 0) {
                ...
                placeBet(bettors[i], marketId, HashPrediction.Outcome.Yes, yesBets[i]);
                totalYes += yesBets[i];
            }
            if (noBets[i] > 0) {
                ...
                vm.prank(bettors[i]);
                stablecoin.approve(address(market), type(uint256).max);
                placeBet(bettors[i], marketId, HashPrediction.Outcome.No, noBets[i]);
                totalNo += noBets[i];
            }
        }
    ...
    function testFuzz_BoundaryValues(uint256 amount) public {
        ...
        uint256 marketId = createDefaultMarket();
        placeBet(alice, marketId, HashPrediction.Outcome.Yes, amount);
        placeBet(bob, marketId, HashPrediction.Outcome.No, amount);
    ...
    function testFuzz_CancelledRefund(uint256 yesBet, uint256 noBet) public {
        ...
        uint256 marketId = createDefaultMarket();
        placeBet(alice, marketId, HashPrediction.Outcome.Yes, yesBet);
        if (noBet > 0) {
            placeBet(alice, marketId, HashPrediction.Outcome.No, noBet);
        }
